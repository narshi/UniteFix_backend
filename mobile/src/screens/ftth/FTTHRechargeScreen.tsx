...
export function FTTHRechargeScreen({ navigation, route }: Props) {
    ...
    const pay = async () => {
        ...
        try {
            ...
            try {
                const verified = await ftthApi.verifyRecharge({
                ...
                Alert.alert(
                    'Recharge successful',
                    verified.data?.validTill
                        ? `Your connection is now valid till ${new Date(verified.data.validTill).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}.`
                        : 'Your connection has been extended.',
                    [{ text: 'Done', onPress: () => navigation.goBack() }],
                );
            } catch {
                // The money is captured; only our confirmation call failed. The
                // webhook will settle it, so do NOT report a failed payment.
                queryClient.invalidateQueries({ queryKey: ['ftth', 'connections'] });
                Alert.alert(
                    'Payment received',
                    'Your recharge is being confirmed and will appear in a moment.',
